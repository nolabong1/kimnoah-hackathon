...
- `source_review_material_view.py`: 붙여넣은 텍스트 또는 PDF 기반 AI 복습자료
  입력과 생성 결과를 좌우 영역으로 표시. PDF는 기본 빠른 추출과 사용자가
  명시적으로 선택하는 AI 정밀 읽기, 추출 품질·근거 확인 상태를 제공
...
3. 사용자가 같은 계획의 원본 텍스트·PDF 추출문 또는 AI 학습자료를 선택하면
   최대 12,000자의 제한된 문맥만 전달한다. 모든 문항은 선택 자료 안에서
...
5. `save_quiz_with_concepts` RPC가 퀴즈, 문항별 자료명·근거와 개념 연결을 저장한다.
