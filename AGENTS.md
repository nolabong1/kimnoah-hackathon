...
이 문서는 2026-09-05 현재 저장소 구조를 기준으로 한다. 문서와 코드가
...
- 확장된 사이드바 탐색을 사용자 목적에 따라 `학습하기`, `AI로 도움받기`,
  `성장 확인하기`, `학습방 꾸미기` 영역으로 그룹화
- `학습하기`의 `시험 대비 모의 평가`에서 저장 계획 전체 범위의 문제 세트를
  생성·반복 응시하며 과제 퀴즈와 공식 사전·사후 평가 상태는 변경하지 않음
- `학습방 꾸미기`에는 `내 학습방` 통합 페이지와 `상점`만 직접 표시하고,
  통합 페이지 안에서 학습방·컬렉션 중 하나만 선택해 렌더링. 별도 내 아이템
  화면은 두지 않고 컬렉션의 전체·보유·장착·미보유 필터로 보유 현황을 확인
  (`/inventory`, `/collection` 기존 주소는 숨김 호환 경로로 통합 화면에 연결)
- 사이드바의 강조된 `빠른 사용 안내` 버튼으로 초보자용 핵심 흐름과 기능 설명 dialog 제공
...
- `models/quiz.py`: 객관식 퀴즈와 문항 모델, 대표 개념·성공 기준·
  선택지별 오답 진단 피드백과 선택 자료의 원문 근거 검증
...
- `models/learning_assessment.py`: 계획별 사전·사후 평행형 객관식 문항,
  서버 채점 결과와 학습목표별 공식 점수, 응시 자격 상태 모델
- `models/mock_exam.py`: 계획 전체 범위의 15문항 모의 평가, 정답 제한 조회,
  반복 응시와 학습목표별 서버 채점 응답 모델
...
- `models/ai_quality.py`: 버전 관리 평가 사례와 재현 입력, 8개 품질 차원,
  오류·경고 검사, 선택 실행 결과와 품질 보고서 모델
- `models/learner_context.py`: AI에 전달할 제한된 우선·안정 개념 숙련도와
  반복 오답 유형 신호 모델
...
- `models/source_material.py`: AI PDF 정밀 읽기의 페이지별 텍스트·시각 정보와
  최대 여덟 장 이미지의 순서·텍스트·시각 관찰·추출 경고 Structured Output 모델
...
- `time_service.py`: `Asia/Seoul` 현재 시각·날짜와 UTC 기준을 한곳에서 제공
- `presentation_labels.py`: 과제·상점·게임화의 공통 한국어 표시 라벨을
  Streamlit view에 의존하지 않는 서비스 경계에서 제공
- `reward_policy.py`: 화면 표시와 연출이 공유하는 과제 10 EXP, 일일 20 EXP,
  레벨당 100 EXP 규칙과 레벨 계산을 제공. 실제 지급 권한은 기존 SQL RPC에 유지하고
  자동 테스트로 Python 표시 규칙과 SQL 지급 규칙의 일치를 확인
...
- `source_material_service.py`: 원본 제목·최대 90,000자 텍스트 검증,
  PDF 일반·레이아웃 이중 추출, 페이지 경계·머리말 정리와 긴 원본 분할
- `pdf_visual_extraction_service.py`: 사용자가 명시적으로 선택한 최대 20페이지
  PDF를 저장하지 않고 Responses API 파일 입력으로 한 번 정밀 읽어
  스캔 텍스트·표·도표·수식 관찰을 페이지별 텍스트로 변환
- `image_input_service.py`: PNG·JPG·JPEG·WEBP 이미지를 파일당 5MB, 요청당
  최대 여덟 장·전체 15MB로 검증하고 EXIF 방향을 적용한 뒤 최대 2048px로
  메타데이터 없이 재인코딩하며, 해시가 검증된 세션 payload와 Responses API
  이미지 입력을 첨부 순서대로 구성
- `image_visual_extraction_service.py`: 저장하지 않은 이미지 입력을
  `detail=high`, `store=False`로 한 번 분석해 원문 텍스트와 표·도표·수식의
  시각 관찰을 검증된 복습자료용 텍스트로 변환
- `review_material_service.py`: 과제·원본 기반 AI 학습자료 생성, 원문 인용을
  내부 검증에만 사용한 교정 재시도와 간결한 본문 변환, 긴 원본 구간별 분석·최종 통합. 연결된 세부
...
- `quiz_service.py`: 검증된 과제 학습목표와 5문항 성공 기준 배분을 반영한
  객관식 퀴즈 생성, 선택 자료 길이 제한, 원문 근거 대조와 업무 규칙 검증
- `quiz_repository.py`: 퀴즈·응시 조회, 개념 포함 퀴즈 저장 RPC,
  목표·참고자료 ID 포함 원자 저장과 제출 RPC 연결. 닫힌 퀴즈의 완료 가능 여부는
  ID만 조회하며 상세 응시는 최근 10개로 제한
...
- `learning_assessment_service.py`: 계획의 2~5개 학습목표마다 설명·적용·오해
  구분 문항을 사전·사후 각 3개씩 한 번에 생성하고 평행형 계약을 재검증
- `learning_assessment_repository.py`: 평가 쌍 원자 저장, 정답을 가린 응시 상태
  조회와 공식 답안 멱등 제출 RPC의 소유권·응답 모델 검증
- `mock_exam_service.py`: 목표·성공 기준·난이도를 균등 배분한 15문항 시험 대비
  모의 평가 생성과 선택 자료 원문 근거 검증
- `mock_exam_repository.py`: 모의 평가 멱등 저장, 본인 목록·정답 제한 상태 조회와
  반복 응시 서버 채점 RPC 연결
- `tutor_service.py`: 튜터 입력·참고자료 길이 검증, 선택 문제 이미지와 함께
  단계별 안내·수정 풀이 피드백을 생성하는 OpenAI 호출
...
- `shop_catalog.py`: 승인된 꾸미기 아이템 15종의 기본값과 배포된 슬롯·레이어·
  에셋 경로 계약 정의. 운영 DB의 이름·가격·희귀도·정렬·활성 상태는 허용하되
  로컬에 없는 에셋 계약은 거부
- `shop_repository.py`: 코인 지갑·상점·인벤토리·학습방 조회와
  구매·학습방 저장·상점 테스트 세션 RPC 연결. 활성 상품 조회 시 DB 행의
  불변 에셋 계약이 배포된 로컬 카탈로그와 일치하는지 검증
- `study_room_service.py`: 보유·슬롯·2D 변형 규칙 검증과 로컬 에셋 기반
  1600×900 학습방 미리보기·직접 편집 장면 구성. 캔버스와 변형 범위는
  모델의 공통 계약을 사용하고 편집기 payload로 전달
...
- `ai_quality_service.py`: 유료 호출 없이 학습계획·학습자료·퀴즈·튜터 결과를
  기능별 3개·총 12개 고정 사례와 결정론적 오류·경고 기준으로 비교하는
  개발용 평가 하네스
- `ai_quality_benchmark_service.py`: 최대 4개 고정 사례를 명시적으로 실제
  생성하고 안전한 로컬 결과 스냅샷과 결정론적 평가 보고서를 조립
- `learner_context_service.py`: 과목별 숙련도를 UUID 없이 최대 6개 우선 개념과
  최대 3개 안정 개념으로 축약하고, 최근 5개 오답 중 2회 이상 반복된 유형만
...
- `learning_objective_repository.py`: 본인의 여러 계획 목표를 한 번에 조회하고
  과제의 실제 DB 연결을 재확인해 생성 서비스에 검증된 목표만 전달. 연결 요약을
  열었을 때만 본인 계획의 자료·퀴즈 최소 필드를 추가 조회
- `learning_objective_connection_service.py`: 저장된 목표 순서대로 과제·원본·
  AI 자료·퀴즈 제목을 묶고 이전 버전 미연결 데이터 수를 계산하는 순수 함수
...
- `help_view.py`: 사이드바에서 여는 초보자용 안내 dialog. 3분 시작·기능 찾기·
  성장과 보상·문제 해결 탭에서 최신 학습 흐름과 실제 메뉴 위치를 안내
- `cursor_style.py`: 승인된 로컬 PNG를 data URL로 변환해 로그인 화면을 포함한
  전역에 게임풍 커서를 적용하고 입력·비활성 요소의 기본 조작성을 보존
- `error_feedback.py`: 내부 예외 원문 대신 안전한 한국어 안내와 오류 ID를
  표시하는 공통 오류·경고 UI helper
- `operation_feedback.py`: 핵심 AI 생성·검증·저장의 현재 단계를 같은
  `st.status` 영역에 표시하고 완료·실패 상태를 일관되게 갱신하는 helper
- `cache_config.py`: 계획·자료·프로필·상점 세션 캐시가 공유하는 30초 TTL 기준
...
- `learning_quest_map_component.py`: 오늘 과제를 시작·과제·완료 노드로 연결해
  진행 상태를 표시하고, 허용된 과제 노드 클릭을 기존 대시보드 과제 선택에만
  반영하는 Components v2 기반 퀘스트 맵
- `learning_momentum_component.py`: 이미 조회한 프로필과 오늘 과제 완료 수로
  연속 학습 불꽃·오늘 진행 원형 게이지·100 EXP 단위 레벨 진행을 표시하는
  읽기 전용 Components v2 HUD. 연속 학습은 0·1·3·7·14·30일 경계의 여섯
  화면 단계로만 표현하며 실제 기록이나 보상 계산을 변경하지 않음
- `streak_presentation.py`: 0·1·3·7·14·30일 연속 학습 화면 단계와 짧은
  라벨을 HUD와 학습방이 함께 사용하는 읽기 전용 표현 규칙
- `focus_sprint_component.py`: 오늘 학습의 선택 과제 학습 단계에서 예상시간을
  최대 25분 단위로 나눠 시작·일시정지·초기화하는 브라우저 집중 타이머.
  타이머 종료 후에는 사용자가 명시적으로 요청한 경우 기존 완료 확인 단계만
  열며, 과제 완료·EXP·실제 학습시간 기록은 변경하지 않음
- `learning_performance_view.py`: 저장된 계획 하나의 과제 완료, 퀴즈 재응시,
  개념별 숙련도 전후 변화와 저장된 학생 직접 회고·AI 회고를 성과 요약·
  공식 학습 전·후 평가·목표별 성과·성장 근거 탭으로 분리해 표시.
  예상시간과 실제 학습시간을
  구분하고 점수 변화와 학습 효과의 인과관계를 단정하지 않으며, 같은 근거를
  추가 호출 없이 브라우저·인쇄용 UTF-8 HTML 리포트로 내려받을 수 있음
- `learning_assessment_state.py`: `learning_assessment_` 접두사 이동·생성 저장
  재시도·답안 제출 멱등 키 상태를 다른 화면 상태와 분리
- `learning_assessment_ui.py`: 학습목표별 공식 사전 진단과 사후 평가 응시,
  서버 채점 결과·문항 해설·목표별 전후 변화와 주간 회고 연결 UI
- `mock_exam_state.py`: `mock_exam_` 접두사 생성 저장 재시도·제출 멱등 상태 관리
- `mock_exam_view.py`: 계획·선택 참고자료 기반 문제 세트 생성, 15문항 응시,
  최근·최고 성적과 보완 학습목표·문항 해설 표시
- `mastery_dashboard_view.py`: 전체 과목 비교와 선택 과목 개념 상세를
  탭으로 분리하고, 선택 과목은 추천 스킬트리와 2열 개념 카드 보기를 제공.
  스킬트리는 실제 선수 관계를 추정하지 않고 취약 상태·숙련도 기반 보완 순서를
  최대 15개 노드로 표시하며 클릭한 노드의 판단 근거를 브라우저 안에서 확인
- `mastery_skill_tree_component.py`: 저장된 숙련도를 결정론적 추천 노드로
  변환하고, DB 재조회 없이 노드 선택과 상세 확인을 처리하는 Components v2 UI
- `saved_plans_view.py`: 저장된 계획/과제 조회, 완료와 삭제 확인.
  ...
  결과와 학습 진단의 좌우 배치를 확보. 실제 계획이 없을 때는 계획 생성 이동
  버튼을 표시하고, 조회 실패는 빈 상태와 섞지 않고 오류만 표시
- `learning_objective_connections_ui.py`: 저장된 계획에서 사용자가 명시적으로
  펼친 경우에만 목표별 연결 과제·원본·AI 자료·퀴즈와 미연결 이전 데이터를 표시
- `review_material_ui.py`: `learn`과 `review` 과제의 자료 생성·저장·조회
- `source_review_material_view.py`: 붙여넣은 텍스트·PDF 또는 이미지 기반 AI 복습자료
  입력과 생성 결과를 좌우 영역으로 표시하고, 계획별 저장 보관함에서 기존
  결과를 다시 조회·확인 후 삭제. PDF는 기본 빠른 추출과 사용자가 명시적으로
  선택하는 AI 정밀 읽기를 제공하며 이미지는 최대 여덟 장을 한 요청으로 읽고
  이미지별 구분이 보존된 추출 텍스트만 저장
- `quiz_ui.py`: 퀴즈 생성·응시·재응시 흐름과 제출 상태를 조합하며 동일 답안의
  제출 재시도에는 기존 멱등 키를 재사용. 생성할 때 과제 목표를 재검증하고
  같은 목표에 연결된 원본·AI 학습자료만 참고자료 선택지로 표시
- `quiz_result_ui.py`: 퀴즈 결과의 문항 해설과 원문 근거, 선택지 오답 진단,
  숙련도 변화·취약 개념·자동 복습 일정을 분리 렌더링. 결과 화면은 왼쪽 문항
  해설과 오른쪽 학습 진단 영역으로 배치
- `completion_feedback.py`: 과제 완료와 일일 보너스 피드백 dialog
- `interaction_state.py`: 서버에서 확정된 과제·퀴즈 결과를 rerun당 한 번만
  재생하도록 제한된 `interaction_` 이벤트 큐와 최근 재생 ID를 관리. 도전과제
  보상은 서버가 신규 EXP 지급을 확정한 수령 결과만 큐에 추가하며, 과제·일일·
  업적·도전 보상 전후 EXP와 기존 공식을 재검증해 실제 레벨 상승만 추가
- `interaction_feedback_component.py`: Components v2 기반 과제 완료·일일 보너스·
  퀴즈 점수·업적·도전과제·레벨업 연속 축하 연출. 테마와 동작 줄이기 설정을 따름
- `interaction_feedback.py`: 기존 게임화 알림과 학습 결과 이벤트를 검증된
  공통 연출 목록으로 조합. 과제 완료 이벤트는 완료 dialog 안에서, 퀴즈 등
  dialog가 없는 이벤트는 전역 오버레이에서 한 번 표시
- `tutor_state.py`: `tutor_` 접두사 세션 상태와 힌트 이동·초기화
- `tutor_view.py`: 2열 설정, 선택 문제 스크린샷, 단계별 힌트 카드, 풀이 점검과
...
- `gamification_view.py`: 일간·주간 도전과제, 업적 진행도, 대표 배지 탭을
  표시하고 오늘 학습 요약도 제공
- `shop_pages_view.py`: 사이드바 `학습방 꾸미기` 그룹의 상점과 내 학습방
  통합 페이지를 조합. 통합 페이지의 학습방·컬렉션 전환 중 선택한
  영역만 렌더링하며 상점 공통 데이터와 필요한 학습방 상태를 한 번씩 조회
- `shop_state.py`: `shop_` 접두사 구매·학습방 저장 처리와 필터·알림 상태 관리
- `study_plan_data_state.py`: 사용자별 저장 계획과 계획별 과제·학습목표 목록을
  30초 동안 세션에서 재사용하고, 계획 생성·삭제 및 과제 완료·퀴즈 제출·테스트 초기화 등
  서버 데이터 변경이 성공하면 관련 캐시만 즉시 무효화
- `reference_material_state.py`: AI 튜터·퀴즈의 계획별 원본·AI 참고자료 목록과
  원본 기반 복습자료 보관함을 사용자 세션에서 30초 재사용하고 자료 변경·삭제 시 무효화
- `shop_view.py`: 코인 지갑, 카테고리별 상점 카드, 구매 확인 dialog와
  영구 보유 인벤토리 UI. 서버가 새 구매를 확정한 경우에만 다음 rerun에서
  일회성 아이템 획득 연출을 표시하며 중복 구매에는 연출하지 않음
- `shop_purchase_reveal_component.py`: 검증된 새 구매의 썸네일·희귀도·사용 및
  잔여 코인을 서버 재호출 없이 한 번 공개하는 Components v2 연출
- `shop_asset_utils.py`: 코드 카탈로그와 일치하는 로컬 상점 썸네일만 Path 또는
  캐시된 data URL로 제공하는 공용 허용목록 helper
- `study_room_view.py`: 왼쪽 직접 조작 미리보기와 오른쪽 일곱 슬롯 선택,
  명시적 학습방 저장 UI. 서버 저장이 확정된 경우에만 편집기 안에서 현재 장착
  수와 변경 내용을 일회성 피드백으로 표시
- `study_room_editor_component.py`: Components v2 기반 클릭·드래그 이동,
  모서리 크기, 회전 핸들, 좌우 반전 학습방 편집기. 이미 조회한 레벨·연속
  학습일의 여섯 화면 단계에 따른 조명·캐릭터·응원 강도, 장착된 공부 고양이의
  상태별 클릭·저장 반응, DB 재실행 없는 짧은 응원과 서버 저장 완료 연출도 제공.
  캔버스와 변형 제한은 Python 장면 payload를 단일 기준으로 사용
- `collection_view.py`: 전체 15종과 카테고리별 수집률, 잠금·보유·장착 상태를
  서버의 기존 상점·인벤토리·학습방 조회 결과로 표시하는 읽기 전용 UI.
  카드를 선택하면 서버 재실행 없이 상세 상태와 상점·학습방 이용 안내를 표시
- `collection_gallery_component.py`: 승인된 로컬 썸네일과 검증된 수집 상태만
  받아 카드 선택·잠금 표현·상세 보기를 처리하는 Components v2 도감
...
- `test_tools_view.py`: 사이드바의 개발용 계획 전체 완료·오늘 기록 초기화·
  상점 테스트 세션과 확인 절차. 연속 학습은 실제 DB·보상 변경 없이
  0·1·3·7·14·30일의 사이드바·오늘 학습·학습방 화면만 미리 볼 수 있음.
  `test_tool_access`에 명시적으로 허용된 사용자에게만 표시하며 닫혀 있을 때는
  관련 데이터를 조회하지 않음
...
`tests/test_reward_policy.py`는 Python의 EXP·레벨 표시 규칙과
`supabase_task_completion.sql`의 서버 지급 규칙이 서로 어긋나지 않는지 검사한다.

`tests/test_prompt_versions.py`는 모든 AI 생성 진입점의 프롬프트 버전이
고정 형식과 고유성을 유지하는지 검사한다.
...
`tests/test_learning_objective_quiz_links.py`는 퀴즈 참고자료 ID 전달과 서버의
과제 목표·자료 소유권·목표 일치·계약 스냅샷 저장 경계를 검사한다.

`tests/test_learning_objective_connections.py`는 목표별 연결 집계 순서와 미연결
호환 데이터, 최소 필드 조회와 닫힌 연결 UI의 추가 조회 방지를 검사한다.

`tests/test_learning_objective_auto_reviews.py`는 자동 복습 목표 상속 trigger의
원인 퀴즈·응시 소유권, 이전 퀴즈 과제 fallback, 기존 행 소유권 백필과
전체 목표 연결 통합 검증 SQL을 검사한다.

`tests/test_learning_assessment.py`는 사전·사후 평행형 측정 계약, AI 생성 입력,
서버 RPC 소유권·공식 1회 제출·멱등 키, 평가 전용 세션 상태와 SQL 보안 계약을
유료 호출이나 원격 DB 없이 검사한다.

`tests/test_mock_exam.py`는 15문항 목표·성공 기준·난이도 배분, Structured Output,
원문 근거, 저장·제출 멱등성, 세션 초기화 범위와 SQL 보안 계약을 검사한다.
...
1. 사용자가 본인의 저장된 계획, 계획 안의 세부 학습목표와 `text`, `pdf` 또는
   `image` 원본을 선택한다.
2. 제목, PDF 10MB, 이미지 파일당 5MB·최대 여덟 장·전체 15MB, 추출 결과와
   최대 90,000자를 OpenAI 호출 전에 검증한다.
3. 기본 PDF 처리는 일반·레이아웃 추출을 비교해 페이지 경계를 보존하고 반복
   머리말·꼬리말을 정리한다. AI 정밀 읽기는 사용자가 직접 선택한 최대
   20페이지 PDF만 `detail=high`, `store=False` 파일 입력으로 한 번 분석한다.
4. 이미지는 PNG·JPG·JPEG·WEBP 최대 여덟 장을 허용하고 EXIF 방향 보정,
   최대 2048px 축소, 메타데이터 제거 재인코딩 후 첨부 순서대로 한 번의
   `detail=high`, `store=False` 요청으로 읽는다. 이미지별 원문 글자와
   표·도표·수식 관찰을 구분된 추출 텍스트로 합치며 원본 바이트는 저장하지 않는다.
5. 30,000자 이하는 한 번 생성하고, 더 긴 원본은 최대 20,000자 구간으로
   손실 없이 나눠 근거가 검증된 부분 자료를 만든 뒤 최종 통합한다.
6. 핵심 개념·세부 내용·주의점·능동 회상 문제의 원문 인용이 실제 추출 텍스트에
   존재하는지 Python에서 재검증하고 실패 시 한 번만 교정 재시도한다.
7. 검증용 원문 인용은 사용자 본문에 반복 노출하지 않고, 핵심 설명·체크리스트·
   회상 문제만 포함한 간결한 고정 한국어 Markdown으로 변환한다. 이전에 저장된
   자료의 반복 인용도 화면 표시 시 제거한다.
8. 생성 성공 후 `learning_materials`에 추출 텍스트와 선택 목표를 저장하고,
   `review_materials.source_material_id`로 결과를 연결한다. DB trigger는 원본의
   목표를 복습자료에 상속하고 생성 당시 목표 계약 스냅샷과 해시를 보존한다.
9. 두 번째 저장이 실패하면 이번 요청에서 생성한 원본 행만 정리한다.

PDF·이미지 원본 파일은 로컬·Supabase에 저장하지 않는다. AI 정밀 읽기 PDF 또는
이미지를 선택한 경우에만 요청 중 OpenAI에 원본 또는 안전하게 재인코딩한 바이트를
전송하며, 이후 저장하는 값은 추출·관찰 텍스트뿐이다.

### 퀴즈와 적응형 학습

1. 퀴즈 과제의 실제 DB 목표 연결과 계약 해시를 다시 확인하고 계획 목표·과제
   범위를 학습자료와 동일한 `LearningBlueprint` 구조로 정규화한다.
2. AI가 객관식 5문항을 만들며 문항마다 대표 개념 하나와 네 선택지에 대응하는
   오답 유형·교정 피드백을 생성한다. 평가된
   개념이 있으면 현재 과제와 관련된 취약·최근 오답 신호를 진단 우선순위로
   사용하고 관련 없는 개념은 강제로 출제하지 않는다.
3. 사용자가 같은 계획·같은 학습목표의 원본 텍스트·PDF 추출문 또는 AI
   학습자료를 선택하면 최대 12,000자의 제한된 문맥만 전달한다. 다른 목표나
   목표가 없는 자료는 선택지에서 제외한다. 모든 문항은 선택 자료 안에서
   확인할 수 있어야 하며 문항별 `source_evidence`가 실제 원문에 존재하는지
   Python에서 재검증한다. 자료가 없는 기존 생성 흐름도 유지한다.
4. `concept_service.py`가 과목·개념 키와 별칭을 정규화한다.
5. 새 9인자 `save_quiz_with_concepts` RPC가 과제 목표를 서버에서 다시 계산하고
   참고자료의 사용자·계획·목표 일치를 확인한 뒤 퀴즈, 생성 당시 목표 계약
   스냅샷, 참고자료 ID, 문항별 자료명·근거와 개념 연결을 원자적으로 저장한다.
   이전 7인자 함수는 내부 구현으로만 사용하며 인증 사용자가 직접 실행할 수 없다.
6. 사용자가 제출하면 클라이언트가 UUID `submission_key`를 보낸다.
7. `submit_quiz_attempt` RPC가 한 트랜잭션에서 채점, 응시 저장,
   문항별 숙련도 갱신, 취약 판정, 자동 복습 생성을 처리한다. 숙련도 이벤트
   삽입 트리거는 저장된 응시 스냅샷과 답안에서 오답 유형을 서버 측으로
   계산하며 클라이언트가 진단 유형을 지정하지 않는다.
   자동 복습은 1일·3일·7일 목표의 최대 3단계로 함께 예약하고 원인 퀴즈 또는
   기능 도입 전 퀴즈 과제의 학습목표를 서버 trigger로 상속한다.
8. 결과 UI는 점수, 정오답, 해설, 선택 자료의 근거 문장, 숙련도 변화,
   취약 개념, 생성된 복습 과제와 예정일을 표시한다.
9. 새로고침 후에도 저장된 응시와 숙련도 원장에서 결과를 재구성한다.
...
1. 새 계획은 과제 완료나 퀴즈 응시 전 세부 학습목표별 사전·사후 평가를
   한 번의 Structured Output 호출로 함께 생성한다.
2. 각 목표는 `explain`, `apply`, `differentiate` 세 성공 기준을 같은 깊이로
   측정하며 사전·사후 문항은 서로 다르게 구성한다.
3. 정답과 해설이 포함된 원본은 클라이언트 직접 조회 권한을 두지 않고,
   `get_learning_assessment_state` RPC가 현재 응시 가능한 문항만 정답 없이 반환한다.
4. 사전 진단은 학습 활동 시작 전에만, 사후 평가는 사전 진단 완료 후 계획
   목표일 도달 또는 모든 과제 완료 시에만 응시할 수 있다.
5. 단계별 공식 응시는 한 번만 저장하며 같은 제출 키·답안 재시도는 기존
   결과를 반환한다. 채점과 자격 판정은 서버 트랜잭션 경계에서 처리한다.
6. 사전·사후 평가는 EXP, 과제 완료, 개념 숙련도와 자동 복습을 변경하지 않는다.
7. 결과는 학습 성과 리포트에서 학습목표별 점수 변화를 관찰 근거로 표시하며
   점수 변화만으로 학습의 인과효과를 단정하지 않는다.

### 시험 대비 모의 평가

1. 사용자는 본인의 저장 계획과 선택적인 원본 또는 AI 복습자료 하나로 새
   15문항 문제 세트를 명시적으로 생성한다.
2. 계획의 2~5개 세부 학습목표를 균등 배분하고 각 목표의 설명·적용·오해
   구분을 모두 다룬다. 난이도는 기초 4, 표준 7, 도전 4문항이다.
3. 선택 자료를 실제로 사용한 문항의 짧은 원문 근거만 저장하며 Python과 서버가
   해당 구절이 선택 자료에 존재하는지 다시 검증한다.
4. 정답과 해설이 든 원본 테이블은 클라이언트 직접 접근을 막고 조회 RPC는
   응시 문항에서 정답·해설·원문 근거를 제거한다.
5. 재응시는 허용하며 서버가 증가하는 응시 번호, 최신·최고 점수와 목표별
   결과를 저장한다. 생성·제출 UUID는 중복 클릭과 네트워크 재시도를 멱등 처리한다.
6. 70점 미만이거나 두 문항 이상 틀린 목표를 해당 응시의 보완 영역으로만
   표시한다.
7. 모의 평가는 EXP, 과제 완료, 공식 사전·사후 평가, 개념 숙련도와 자동 복습
   일정을 변경하지 않는다. 권장 시간은 안내용이며 자동 제출하지 않는다.
...
8. 상품명·가격·희귀도·정렬·활성 여부는 운영 DB를 기준으로 하고,
   슬롯·레이어·오버레이·썸네일 경로는 배포 코드의 불변 에셋 계약과 일치해야 한다.
...
6. 책상·의자·좌우 소품·포인트는 브라우저 편집기에서 직접 이동·크기·회전·
   ...
   캔버스 크기와 허용 변형 범위는 `models/shop.py`를 단일 기준으로 사용한다.
...
1. 컬렉션은 활성 `shop_items`와 본인 `user_inventory`, 저장된
   `user_study_rooms` 조회 결과만 재사용하며 별도 테이블이나 쓰기 RPC를 쓰지 않는다.
2. 전체 수집률과 카테고리별 수집률은 활성 카탈로그의 고유 아이템 수를 분모로
   계산하고 무료 기본 방은 포함하지 않는다.
3. 현재 장착 수는 보유한 활성 아이템 중 저장된 일곱 슬롯에 있는 고유 아이템
   수이며 같은 키가 잘못 반복돼도 한 번만 센다.
4. 컬렉션 화면은 구매·장착·EXP·코인·과제 상태를 변경하지 않는 읽기 전용이다.

### 단계별 힌트 AI 튜터

1. 사용자가 본인의 계획과 선택 과제·참고자료, 문제 글 또는 문제 이미지 최대 여덟 장,
   현재 풀이를 입력한다.
...
4. 수정 풀이 점검을 명시적으로 제출할 때만 문제 이미지를 함께 보내 별도의
   OpenAI 호출을 한 번 한다.
...
설정 위젯은 모두 `tutor_` 접두사를 사용한다. 문제 이미지는 PNG·JPG·JPEG·WEBP를
파일당 5MB, 요청당 최대 여덟 장·전체 15MB로 받고 최대 2048px로 재인코딩해
첨부 순서대로 활성 세션에만 두며, 새 질문 또는 로그아웃 시 튜터 상태와 함께 제거한다.
...
- `learning_materials`: 붙여넣은 원본 또는 PDF·이미지에서 추출한 텍스트
- `review_materials`: 과제 또는 사용자 원본 기반 AI 생성 학습자료
- `quizzes`: 과제별 현재 퀴즈와 JSON 문항
- `quiz_attempts`: 재응시별 답안, 스냅샷, 점수, 제출 식별 키
- `learning_assessments`: 계획별 사전·사후 평행형 평가 원본과 학습목표 스냅샷
- `learning_assessment_attempts`: 단계별 공식 1회 답안, 서버 채점과 목표별 점수
- `mock_exams`: 계획 전체 범위의 15문항 모의 평가와 목표·참고자료 스냅샷
- `mock_exam_attempts`: 반복 응시별 답안, 문항·학습목표별 서버 채점 결과
...
- `concept_mastery_events`: 응시 문항별 숙련도 변경과 선택지 기반 오답 유형 원장
...
- 학습목표 DB 기반은 nullable 연결 열, 기존 계획 백필, 본인 조회 RLS,
  ...
  DB trigger로 상속하고 생성 목표의 계약 스냅샷을 보존한다. 새 퀴즈는 과제 목표와
  같은 목표의 참고자료만 연결하고 생성 당시 목표 계약과 자료 ID를 함께 보존한다.
- 퀴즈와 응시는 계획·과제·사용자 소유권을 함께 검증한다.
- 사전·사후 평가는 `(plan_id, user_id)` 복합 외래 키로 본인 계획에 연결되고,
  응시는 `(assessment_id, user_id)`로 같은 사용자의 평가만 참조한다.
- 모의 평가도 `(plan_id, user_id)`로 본인 계획과 선택 자료를 검증하고, 응시는
  `(mock_exam_id, user_id)`로 같은 사용자의 문제 세트만 참조한다.
- 숙련도는 `(user_id, concept_id)` 단위이다.
- 숙련도 이벤트는 사용자, 퀴즈, 응시, 문항 인덱스, 개념과 검증된 오답 유형을
  연결한다. 정답과 진단 피드백이 없는 과거 문항의 오답 유형은 `NULL`이다.
...
- 보상, 채점, 숙련도 변경, 자동 복습, 테스트 초기화는 서버 RPC에서
  원자적으로 처리한다.
- 공식 사전·사후 평가 원본과 답안 테이블은 authenticated 직접 접근을 막고,
  소유권과 응시 시점을 검증하는 전용 RPC만 실행하도록 허용한다.
- 모의 평가 원본과 응시 테이블도 authenticated 직접 접근을 막고, 소유권을
  검증하며 정답을 숨기는 조회·저장·채점 RPC만 실행하도록 허용한다.
...
- 이미지 원본 파일도 저장하지 않고 AI가 추출한 텍스트만 저장한다.
- PDF 자료의 `material_type`은 `pdf`, 이미지는 `image`, 붙여넣기 자료는 `text`이다.
- PDF 기본 모드는 로컬 텍스트 추출이며, 스캔본·표·도표·수식은 사용자가
  명시적으로 고른 AI 정밀 읽기에서만 처리한다.
...
- 새 퀴즈 문항은 선택지별 진단 피드백 4개를 가지며 정답 선택지만
  `correct_reasoning`을 사용한다. 과거 피드백 없는 퀴즈는 기존 해설로 표시한다.
...
- AI 개인화에는 개념별 최근 오답 5개 중 같은 유형이 2회 이상 나타난 경우만
  사용하며 최대 2개 유형까지만 전달한다. 이 신호는 숙련도 계산이나 취약
  판정 기준을 변경하지 않는다.
...
- AI 생성 진입점은 서비스별 명시적 프롬프트 버전 상수를 유지하며,
  품질 평가 fixture와 버전 이름이 어긋나지 않도록 자동 테스트한다.
...
마지막으로 `supabase_learning_objective_quiz_links.sql`과 해당 validation으로
퀴즈 목표·참고자료 원자 연결과 이전 7인자 저장 RPC의 공개 실행 차단을 검증한다.
이어서 `supabase_learning_objective_auto_reviews.sql`과 해당 validation으로
자동 복습 목표 상속과 기존 행 정규화를 검증한다. 모든 연결
단계 적용 후 `supabase_learning_objective_integration_validation.sql`을 실행한다.
이미지 원본 복습자료를 사용하려면 그 다음
`supabase_image_source_material.sql`을 적용하고
`supabase_image_source_material_validation.sql`로 허용 유형, 추출 텍스트 필수,
원본 파일 비저장 제약과 기존 RLS 활성 상태를 검증한다.
...
- AI 학습자료 버전 기록
- PDF·이미지 원본 파일의 Storage 저장과 버전 관리
- 모바일 전용 UI 최적화
